 ...
 * 
 */
function getGridJsonList(_tableId, _url, _isSingleSelect, _footPager, _queryParams, _onLoadSuccess, isColsThead)
{
	...
	if (footPage.size() == 0)
	{
		var footPageHtml = "<div id=\"" + _tableId + "footPage\" class=\"foot-page\"></div>";
		$("#" + _tableId).after(footPageHtml);
	}

	// JYC 2014 12 27 第列添加编号
	var firstobj = $("#" + _tableId).find("thead:first-child>tr>th:first-child");
	if ($(firstobj).attr("name") != 'dgno')
	{
		if (isColsThead)
		{
			$(firstobj).before("<th rowspan=\"2\" name='dgno' data-options=\"field:'dgno',align:'Center',width:42\">序号</th>");
		}
		else
		{
			$(firstobj).before("<th  name='dgno' data-options=\"field:'dgno',align:'Center',width:42\">序号</th>");
		}
	}
	$("#" + _tableId).datagrid({
		url : _url,
		...
		loadFilter : function(data)
		{
			...
			for (var i = 0; i < data.rows.length; i++)
			{
				...
				{
					// 计算编号
					var tempIndex = (footPager.options.pageNum - 1) * footPager.options.pageSize;
		...
		onBeforeLoad : function(param)
		{
			...
			/*
			 * $(this).find(".datagrid-header-rownumber").html('&nbsp;&nbsp;序号'); $(this).find(".datagrid-header-rownumber").css({ 'taxt-align' : 'left' });
			 * $(this).find(".datagrid-cell-rownumber").css({ 'width' : '40px' }); $(this).find(".datagrid-header-rownumber").css({ 'width' : '40px' });
			 */
		...
		onLoadSuccess : function(data)
		{
			...
			/*
			 * $(this).parent().find(".datagrid-cell-rownumber").each(function() { var rowIndex = tempIndex + parseInt($(this).text()); $(this).text(rowIndex);
			 * });
			 */
			/*
			 * $(this).parent().find(".datagrid-header-rownumber").html('&nbsp;&nbsp;序号'); $(this).parent().find(".datagrid-header-rownumber").css({
			 * 'taxt-align' : 'left' }); $(this).parent().find(".datagrid-cell-rownumber").css({ 'width' : '40px' });
			 * $(this).parent().find(".datagrid-header-rownumber").css({ 'width' : '40px' });
			 */
			$(this).parent().find('.datagrid-body').height("auto");
			$(this).parent().find('.datagrid-body').css("overflow-y", "hidden");
			// jyc 2014-12-4 列头全部剧中
			// $(this).parent().find('.datagrid-htable
			// div').css("text-align", "left");
			$(this).parent().find('.datagrid-htable div.datagrid-cell-c1-dgno').css("text-align", "center");
			//window.setTimeout(scrollToSelect, 500);
			if (_onLoadSuccess)
			{
				_onLoadSuccess();
			}
			$('.toggle-tooltip').tooltip({position: 'top'});
		},
...
//			}
	...
	$(window).on("resize",function(){
		$("#" + _tableId).datagrid('resize', {width:datagridResizeWidth});
	});
}
/*
 * 没有查询到记录提示
...
var myview = $.extend({}, $.fn.datagrid.defaults.view, {
	onAfterRender : function(target)
	{
		...
		if (!$(target).datagrid('getRows').length)
		{
			...
			emptyMsgHtml += "		</tr>";
			emptyMsgHtml += "</table>";
			var d = $("<div class='datagrid-empty' style='height:70px;margin-top:80px;text-align: center;border:0px solid black;' ></div>").html(emptyMsgHtml).appendTo(vc);

			// var d = $('<div class="datagrid-empty" ></div>').html(opts.emptyMsg).appendTo(vc);
			/*
			 * d.css({ position : 'absolute', left : 0, top : 60, width : '100%', textAlign : 'center' });
			 */
			$(vc).css("height", 150);
		}
		else
...
function loadPageByForm(url)
{
	...
	if (url.indexOf("?") > -1)
	{
		...
		if (paraArr != null)
		{
			for (var i = 0; i < paraArr.length; i++)
			{
				...
				if (eName != null)
				{
					form.append("<input type='hidden' name='" + eName + "' value='" + eValue + "'/>");
				}
			}
		}
	...
	{	showSubmitLoading("提示信息", "正在加载数据，请稍后", 10, 500, 100);//显示 提交 提示
		...
		form.append("<input type='hidden' name='jieDianId' value='" + _jieDianId + "'/>");
...
function initAjax(url, data, async, dataType, successFunction, errorFunction)
{
	$.ajax({
		...
		error : function(XMLHttpRequest, textStatus, errorThrown)
		{
			...
			{
				showErrorMsg("网络错误,请重试", 150, 230, 30);
			}
		}
	});
}